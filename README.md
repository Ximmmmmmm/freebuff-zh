...
- **多开**：主实例 + 槽位 1~9，互不干扰
- **多账号**：每个槽位独立 Chromium 配置和登录态，可分别登录不同账号
- **状态一览**：实时显示每个实例的运行状态和当前登录的账号邮箱
- **额度显示**：每个账号每日高级会话剩余额度（调用官方会话接口，启动时与手动刷新时更新，另有 5 分钟自动刷新）
- **一键操作**：启动 / 停止 / 重置账号 / 停止全部；双击表格行直接启动
- **两种启动方式**：全新登录（不同账号）或复制主实例账号
- **重置换号**：清空某个槽位即可换登录另一个账号
- **最小化到托盘**，暗色主题 UI，单文件 exe（约 22 KB，无运行时依赖）

## 使用 / Usage

1. 双击 `Freebuff多开控制器.exe`（或按下面步骤自行编译）
2. 选中一个未初始化的槽位，选择「全新登录」，点「启动」
3. 在弹出的 Freebuff 窗口里登录该窗口要用的账号 —— 登录态会固定在这个槽位
4. 想换某个槽位的账号：选中 → 「重置账号」→ 再启动登录新账号
