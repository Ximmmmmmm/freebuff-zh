...
[![Target](https://img.shields.io/badge/目标-Freebuff%20Desktop%20v0.0.136-blue)](https://freebuff.com)
...
Freebuff Desktop（`@codebuff/freebuff-desktop` v0.0.136）的**简体中文汉化包**，直接修改已打包产物，无需源码、不涉及任何联网改动。
...
- **覆盖全面**：渲染进程约 1985 处文案 + 主进程菜单 / 对话框 / 同意窗口全面中文化；connectors / MCP 面板（状态标签、详情面板与目录里 100 条连接器介绍）与 v0.0.104 新增的 BYOK（自带密钥）API 提供商界面已全量中文化
- **词典驱动**：`dict.json`（exact 1324 / template 238 / code 15 / pattern 68），幂等应用、可审计
- **可复现构建**：`build.sh` 从原版 + 词典 + 补丁**逐字节重建**汉化产物  （v0.0.77 曾对照 Release 产物验证；v0.0.83 / v0.0.87 / v0.0.88 / v0.0.90 / v0.0.91 / v0.0.92 / v0.0.103 / v0.0.104 / v0.0.105 / v0.0.106 / v0.0.107 / v0.0.108 / v0.0.109 / v0.0.110 / v0.0.112 / v0.0.113 / v0.0.114 / v0.0.120 / v0.0.123 / v0.0.124 / v0.0.126 / v0.0.127 / v0.0.128 / v0.0.131 / v0.0.132 / v0.0.133 / v0.0.134 / v0.0.135 / v0.0.136 适配经防呆自检通过）
- **v0.0.136 适配**：上游动的是**模型目录**与一条 Codex 更新回执：新增 `Opus 5.5`（`id:"claude-opus-5-5"`，旧 `Opus 5` 下线但留了迁移映射 `t==="claude-opus-5"?xO:t`）；老模型数组由 `o4` 改名 `wO` 并把 5.6 那三条换成 6 系列——`GPT-5.6-Sol` → `GPT-6-Sol`、`GPT-5.6-Luna` → `GPT-6-Luna`，`gpt-5.6-terra` **整条删掉**，且 `iq()` 现在把 `id.startsWith("gpt-5.6")` 的一律过滤（`retiring`）；Codex 更新成功的那句回执由模板 `Codex updated — ${label} is ready to pick` 改成字面量 `Codex updated — model availability refreshed`（另一条 `Codex updated — send your message again.` 仍在）。模型名按约定保留英文，所以**本版真正要翻的新文案为 0 条**，只新增 1 条 exact（那条新 toast）。对差账：片段级 1380 → 1379（新增 1 / 下线 2 / 疑似改写 0）、字面量级 1932 → 1931（新增 1）；`electron/` 下 37 个文件与上一版**逐字节相同**（`diff -rq` 无输出），12 个补丁**一次干净套用**；`ui/` 侧只有主 bundle 变了（`index-C_S8h05L.js` → `index-BY1Nix4m.js`）。词典 exact 1324（新增 1 / 删 1）/ template 239 → 238（共 1645 条），替换数 1986 → 1985（下线两条各 −1、新 toast +1，账对得上）、`all keys matched`、`missed_diagnose` 1645 条全命中；模板变量自动重映射 24 条（`RENAMED` 24 / 歧义 2 / MISSING 1），**两条歧义就是那对 `Dismiss notification` 拷贝又被改了插值名**（`${s.message}`/`${v.message}` → `${l.message}`/`${S.message}`，同一锚文本命中 2 处、插值不一致，`remap` 按设计拒绝写回，逐条人工改名）。`update.sh` 七步全绿：上游新增文案词典已全覆盖、回归闸门 220 vs 220 新增 0 处、单词级文案差集 0 处、字面量占用 0 条、`mainscan` 零漏翻、UI 行为补丁两组 `KEEP`、两道行为取证通过；残留 `uipos` 17 → **16** 条（全部已登记）/ `fieldscan` 1 条 / `blindscan` 279 条。登记表随本版增 3 条（`GPT-6-Sol` / `GPT-6-Luna` / `Opus 5.5`）、清理 4 条已下线的模型名（`GPT-5.6-*` 三条与 `Opus 5`），**`GPT-6 Luna` 仍照旧别删**（它在 `displayName` 里、不在属性位，是给 `upstreamdiff` 用的）
- **v0.0.135 适配**：上游这一版只做了一件事——新增 **`GPT-6 Luna` 模型**（跑在 OpenAI flex 容量上）：模型目录里多一条 `displayName:"GPT-6 Luna"`（模型名按约定保留英文，登进登记表 `uiStrings`）、tagline 复用已有的 `Strong all-around`，真正要翻的新文案只有它那条 tooltip：`Runs on OpenAI flex capacity: half the token price, the same speed in our measurements, and a standard-tier backup when flex is busy.`。对差账：片段级 1379 → 1380（新增 1 / 下线 0 / 疑似改写 0），字面量级 1931 → 1932（新增 1，就是那个模型名——它不属于界面属性位、片段级看不见，只有字面量级报得到）；`electron/` 下 37 个文件只有 `shell-lifetime.cjs` 与上一版不同（心跳间隔 1s → 3s 的省电改动，纯行为、无用户可见文案），12 个补丁**一次干净套用**（本版没动补丁覆盖区，无需重锚定）。词典 exact 1323 → 1324（共 1646 条），替换数 1984 → 1986、`all keys matched`、`missed_diagnose` 1646 条全命中；模板变量自动重映射 12 条（`RENAMED` 12 / 歧义 0 / MISSING 0：`I_.length` → `B_.length`、`Nz(T)` → `Qz(T)`、`bbe(t.limit)` → `_be(t.limit)`、`Lve(new Date(e),n)` → `jve(new Date(e),n)` 这类压缩短名）。`update.sh` 七步全绿：上游新增文案词典已全覆盖、回归闸门 220 vs 220 新增 0 处、单词级文案差集 0 处、字面量占用 0 条、`mainscan` 零漏翻、UI 行为补丁两组 `KEEP`、两道行为取证通过；残留扫描与上一版持平（`uipos` 17 条全部已登记 / `fieldscan` 1 条 / `blindscan` 279 条）。**记一笔口径**：`uipos_gap` 会把新登记的 `GPT-6 Luna` 列进「登记表里有 1 条在本版产物里已看不见，可以清理」——它只看界面属性位，而这条是给 `upstreamdiff` 用的（模型名在模型目录对象的 `displayName` 里，不在属性位），**不要删**；另一件顺手修的事：`shell-lifetime.cjs` 变了但既不加补丁也不进 `mainscan` 名单（新代码只有注释与 `DEFAULT_PING_MS`）
- **v0.0.134 适配**：上游动的是**项目侧栏**：新增 `sidebar-layout-toggle`（`label:t?"Expand left sidebar":"Collapse left sidebar"`）与可拖拽分隔条（`aria-label:"Resize left sidebar"`），侧栏上多了余额小徽章 `sidebar-balance`（`aria-label` 是三元：`` `${Qf(t.balance)} Freebucks` `` / `Freebucks balance unavailable`，tooltip 复用已有的「今日剩余 … + 钱包余额」那句）；另有 `Projects · Open project` 与 `Not enough room left in the message for that skill` 两条新文案。**上游把侧栏那批组件在 bundle 里重复打进了一份**（第二份的压缩短名不同），于是 `Dismiss notification` / `Project:` / `Close` / `Freebucks / hour` / `N pixels` / `Remove ${…configKey}` 各有两种形态——`remap` 报的 6 条 AMBIGUOUS 其实不是歧义而是两份拷贝，7 条词条按新形态改名，两份价钱 / 像素文案各补一条 template（`` `${Math.round(f)} pixels` `` / `Regular price: ${ut} Freebucks / hour`）；`Remove project ${ge}` 那条是上游整段改写（确认态内联成 `` L===de?"Confirm delete":"Remove project" ``），改由 exact 的 `Confirm delete` / `Remove project` 覆盖、旧词条下线。对差账：片段级 1374 → 1379（新增 5）、字面量级 1924 → 1931（新增 7，其中 2 条片段级没报到），`electron/` 仍 37 个文件、12 个补丁**一次干净套用**（本版没动补丁覆盖区，无需重锚定）。词典 exact 1316 → 1323 / template 237 → 239（共 1645 条），替换数 1967 → 1984、`all keys matched`、`missed_diagnose` 1645 条全命中；残留扫描 uipos 15 条（全部已登记）/ `blindscan` 279 条（库内部），主进程零漏翻，UI 行为补丁两组 `KEEP`；`upstreamdiff` 待补翻 0 条、疑似改写 0 组。这一版又给对差提取器补了一条判据：**引号紧贴逗号 / 冒号**（`,"` 与 `"\s*:`）——新徽章把属性表摆在了两个模板之间（`` `… Freebucks`:"Freebucks balance unavailable","data-tooltip":t?`…` ``），模板分段通道抽出的那条 `:"Freebucks balance unavailable","data-tooltip":t?` 既无反引号也无 `void`、前三十几个字符还像句正常英文，于是挤进待补翻、把旁边真该翻的字面量淹在后面；实测四份 bundle 里符合这两条形态的片段各只有这一条，真文案在字面量级照旧抽到（`test_upstreamdiff` 第 17 组把「旧口径会放行它」与「旁边的真字面量仍进待补翻」都钉住）
