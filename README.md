...
[![Target](https://img.shields.io/badge/目标-Freebuff%20Desktop%20v0.0.74-blue)](https://freebuff.com)
...
Freebuff Desktop（`@codebuff/freebuff-desktop` v0.0.74）的**简体中文汉化包**，直接修改已打包产物，无需源码、不涉及任何联网改动。
...
- **覆盖全面**：渲染进程约 790 处文案 + 主进程菜单 / 对话框 / 同意窗口全面中文化
- **词典驱动**：`dict.json`（exact 748 / template 91 / code 4 / pattern 3），幂等应用、可审计
- **可复现构建**：`build.sh` 从原版 + 词典 + 补丁**逐字节重建**汉化产物（已对 v0.0.74 验证）
...
构建管线（已对 v0.0.74 验证，`ui` 与主进程内容与 Release 产物逐字节一致）：
